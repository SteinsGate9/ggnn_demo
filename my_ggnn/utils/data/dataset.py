...
def data_convert(data_list, n_annotation_dim):
    node_num = find_max_node_id(data_list)
    ...
    for item in data_list:
        ...
        for target in target_list:
            ...
            annotation = np.zeros([node_num, n_annotation_dim])
            annotation[target[1]-1][0] = 1
            task_data_list[task_type-1].append([edge_list, annotation, task_output])
    return task_data_list

def create_adjacency_matrix(edges, node_num, edge_type_num):
    a = np.zeros([node_num, node_num * edge_type_num * 2])
    for edge in edges:
        src_idx = edge[0]
        e_type = edge[1]
        tgt_idx = edge[2]
        a[tgt_idx-1][(e_type - 1) * node_num + src_idx - 1] =  1
        a[src_idx-1][(e_type - 1 + edge_type_num) * node_num + tgt_idx - 1] =  1
    return a
...
class bAbIDataset():
    ...
    def __init__(self, path, task_id, is_train):
        ...
        self.edge_type_num =  find_max_edge_id(all_data)
    ...
    def __getitem__(self, index):
        am = create_adjacency_matrix(self.data[index][0], self.node_num, self.edge_type_num)
